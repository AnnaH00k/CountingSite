...
import { Car, Truck, Bus, Bike, Zap, Boxes } from 'lucide-react';
...
export default function TrafficCounter() {
  ...
  const saveCountsToCookies = (counts: CountsState) => {
    ...
    try {
      ...
      document.cookie = `trafficCounts=${encodeURIComponent(JSON.stringify(counts))}; expires=${expiryDate.toUTCString()}; path=/; SameSite=Lax`;
    } catch (error) {
  ...
  const updateCount = (category: CategoryId, direction: Direction) => {
    ...
    setLastAction(`${direction === 'einfahrend' ? 'Einfahrend' : 'Ausfahrend'} - ${categories[category].name}`);
  };
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      } else if (activeKeys.has('a') && ['1', '2', '3', '4', '5', '6'].includes(key)) {
        ...
        <div className="max-w-6xl mx-auto">
          <div className="bg-gray-800 rounded-lg shadow-xl p-6 mb-6">
            <div className="text-center">
              <h1 className="text-3xl font-bold text-white mb-2">Verkehrszähler</h1>
        ...
        <div className="bg-gray-800 rounded-lg shadow-xl p-6 mb-6">
          <div className="text-center mb-6">
            ...
            <p className="text-gray-300 mb-4">
              Tastenkombinationen: <span className="font-mono bg-gray-700 text-gray-200 px-2 py-1 rounded">E + 1-6</span> für Einfahrend, 
              <span className="font-mono bg-gray-700 text-gray-200 px-2 py-1 rounded ml-2">A + 1-6</span> für Ausfahrend, 
              <span className="font-mono bg-gray-700 text-gray-200 px-2 py-1 rounded ml-2">R</span> zum Zurücksetzen
              <br />
              <span className="text-sm text-gray-400 mt-2 block">💾 Daten werden automatisch gespeichert</span>
            </p>
            {lastAction && (
              <div className="bg-blue-900 border border-blue-700 rounded-lg p-3 mb-4">
                <p className="text-blue-200 font-medium">Letzte Aktion: {lastAction}</p>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div className="bg-green-800 border border-green-600 rounded-lg p-4 text-center">
              <h3 className="text-lg font-semibold text-green-200">Einfahrend Gesamt</h3>
              <p className="text-3xl font-bold text-green-100">{totals.einfahrend}</p>
            </div>
            <div className="bg-red-800 border border-red-600 rounded-lg p-4 text-center">
              <h3 className="text-lg font-semibold text-red-200">Ausfahrend Gesamt</h3>
              <p className="text-3xl font-bold text-red-100">{totals.ausfahrend}</p>
            </div>
            <div className="bg-gray-700 border border-gray-600 rounded-lg p-4 text-center">
              ...
              <p className="text-3xl font-bold text-gray-100">{totals.einfahrend + totals.ausfahrend}</p>
            </div>
          </div>

          {/* Kategorien */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {(Object.entries(categories) as [string, Category][]).map(([keyStr, category]) => {
              const key = parseInt(keyStr) as CategoryId;
              const IconComponent = category.icon;
              return (
                <div key={key} className="bg-gray-700 rounded-lg p-4 border border-gray-600">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-100">{key}. {category.name}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="text-center">
                      ...
                      >
                        <div className="text-2xl font-bold">{counts[key].einfahrend}</div>
                        <div className="text-sm">Einfahrend</div>
                        <div className="text-xs mt-1 opacity-75">E + {key}</div>
                      </button>
                    </div>
                    ...
                    <div className="text-center">
                      ...
                      >
                        <div className="text-2xl font-bold">{counts[key].ausfahrend}</div>
                        <div className="text-sm">Ausfahrend</div>
                        <div className="text-xs mt-1 opacity-75">A + {key}</div>
                      </button>
                    </div>
                  </div>
                </div>
            ...
            <div className="mt-4 text-center">
              <div className="bg-yellow-900 border border-yellow-700 rounded-lg p-2 inline-block">
                <span className="text-yellow-200 text-sm">
                  Aktive Tasten: {Array.from(activeKeys).map((k: string) => k.toUpperCase()).join(' + ')}
                </span>
              </div>
            </div>
          )}
